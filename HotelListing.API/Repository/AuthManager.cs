    ...
    public class AuthManager : IAuthManager
    {
        ...
        public async Task<AuthResponseDto> Login(LoginDto loginDto)
        {
            ...
            return new AuthResponseDto
            {
                Token = token,
                UserId = _user.Id,
                RefeshToken = await CreateRefreshToken()
            };
        }
        ...
        public async Task<AuthResponseDto> VerifyRefreshToken(AuthResponseDto request)
        {
            ...
            _user = await _userManager.FindByNameAsync(username.Value);
            if(_user == null || _user.Id != request.UserId)
            { 
                return null;
            }
            var isValidRefreshToken = await _userManager.VerifyUserTokenAsync(_user,
                _loginProvider, _refreshToken, request.RefeshToken);
